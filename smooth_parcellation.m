function [i_cls_smooth] = smooth_parcellation(i_cls,k_num)

% smooth
i_cls_smooth=i_cls; %just make the correct size
...
while length(missing) > length(medial_wall_inds)
 ...
 i_cls_smooth(indsStart(1)+1:indsEnd(1))=final_left(ciftiindsL);
...
while length(missing) > length(medial_wall_inds)
 ...
 i_cls_smooth(indsStart(2)+1:indsEnd(2))=final_right(ciftiindsR);
end 
 %return i_cls_smooth